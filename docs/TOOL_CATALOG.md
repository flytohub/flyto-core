...
| `browser.click` | Click a visible element by its button/link name, page text, ID, or an advanced selector. | `click_method` select (default: `text`), `target` string, `selector` string, `button` select (default: `left`), `click_count` number (default: `1`), `force` boolean (default: `False`), `modifiers` array, `timeout` number (default: `30000`) | `browser` (object), `status` (string), `selector` (string), `method` (string) |
