...
async def pdf_fill_form(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _do_fill():
        ...
        fields_filled = _fill_form_fields(writer, reader, fields)
        images_inserted = _insert_form_images(writer, images)
        with open(output_path, 'wb') as f:
            writer.write(f)
        return fields_filled, images_inserted

    fields_filled, images_inserted = await asyncio.to_thread(_do_fill)
...
def _fill_form_fields(writer, reader, fields: Dict[str, Any]) -> int:
    fields_filled = 0
...
def _insert_form_images(writer, images: List[Dict[str, Any]]) -> int:
    if not images:
        return 0
    ...
    for img_config in images:
        inserted = _insert_single_image(writer, img_config, canvas, BytesIO)
        if inserted:
            images_inserted += 1
    return images_inserted


def _insert_single_image(writer, img_config: Dict[str, Any], canvas, BytesIO) -> bool:
